...
namespace Contracts
{
    internal sealed class ValueCatalogWindow : EditorWindow
    {
        ...
        {
            this.titleContent = new GUIContent("Contract Values");
        }

        private void OnGUI()
        {
            this.catalog = EditorTools.GetValueCatalog();
            ...
            if (_categoryIndex >= this.catalog.categories.Count)
            {
                _categoryIndex = 0;
            }
        ...
        private void DrawHeader()
        {
            ...
            rect.x += 5;
            GUI.Label(rect, " Contract Values:", labelStyle);
        ...
        private void DrawAddCategoryButton()
        {
            ...
            if (!this.catalog.CategoryExists(_newCategoryName) && Regex.IsMatch(_newCategoryName, namePattern))
            {
                ...
                {
                    this.catalog.AddCategory(_newCategoryName);

                    _newCategoryName = DEFAULT_CATEGORY_NAME;
                    _categoryIndex = this.catalog.categories.Count - 1;
        ...
        {
            if (this.catalog.categories.Count == 0 ||
                _categoryIndex < 0 ||
                _categoryIndex >= this.catalog.categories.Count)
            {
                return;
            }
        ...
        private void DrawCustomElement(int itemIndex)
        {
            ...
            const string namePattern = @"^[a-zA-Z_][a-zA-Z0-9_]*$";
            bool isUniqueueName = this.catalog.IsUniqueueName(_categoryIndex, name.stringValue);
        ...
        private void DrawAddElementButton()
        {
            ...
            if (Regex.IsMatch(_newItemName, namePattern) &&
                !this.catalog.NameExists(_categoryIndex, _newItemName))
            {
                GUI.color = new Color(0f, 0.83f, 1f, 1);
                ...
                if (GUILayout.Button("+", GUILayout.Width(50)))
                {
                    this.catalog.AddItem(_categoryIndex, freeId, _newItemName, _newItemType);
        ...
        private void DrawCatalogButton()
        {
            ...
            {
                EditorTools.CreateValueCatalog();
            }
        ...
        private void DrawCompileButton()
        {
            ...
            else
            {
                ...
                {
                    IndexAPIGenerator.Generate(this.catalog);
                }
            }
