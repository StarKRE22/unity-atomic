    ...
    internal static class TagAPIGenerator
    {
        ...
        internal static void Generate(TagsConfig catalog, bool refresh = true)
        {
            ...
            for (int i = 0, count = items.Count; i < count; i++)
            {
                TagsConfig.Item item = items[i];
                writer.WriteLine(
                    $"        public static bool Has{item.type}Tag(this {OBJECT_CLASS} obj) => obj.HasTag({item.type});");
                writer.WriteLine(
                    $"        public static bool Add{item.type}Tag(this {OBJECT_CLASS} obj) => obj.AddTag({item.type});");
                writer.WriteLine(
                    $"        public static bool Del{item.type}Tag(this {OBJECT_CLASS} obj) => obj.DelTag({item.type});");
